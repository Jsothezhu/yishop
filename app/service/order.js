...
class orderService extends Service {
  ...
  async getDetail({ order_id }) {
    ...
    for (const item of orderitem) {
      const good = await ctx.model.Goods.findOne({
        where: {
    ...
    if (status === 'all') {
      try {
        ...
        for (const item1 of order) {
          // 遍历订单项
          for (const item2 of item1.dataValues.order_items) {
            // 查询商品信息
            const goodInfo = await ctx.model.Goods.findOne({
              where: {
  ...
  async cancelOrder(order_id) {
    ...
    if (order.dataValues.ship_status === 'pay') {
