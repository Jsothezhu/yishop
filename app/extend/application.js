...
const handles = {};// 定义处理任务列表
module.exports = {
  /**
   * @description 添加订单延时任务
   */
  async addTask(id, delay = 1800, task = 'order_task') {
    await this.redis.get('default').setex(id, delay, task, error => {
      if (error) {
        console.log('任务设置失败');
      } else {
        console.log('任务设置成功');
  ...
  initDelayTask() {
    ...
    this.redis.get('subscribe').on('pmessage', (channel, message, key) => {
